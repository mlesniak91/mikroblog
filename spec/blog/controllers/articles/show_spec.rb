...
module Blog::Controllers::Articles
  describe Show do
    let(:article_repository) { Minitest::Mock.new }
    let(:comment_repository) { Minitest::Mock.new }
    let(:article) { Object.new }
    let(:comments) {[] }
    let(:action) { Blog::Controllers::Articles::Show.new(article_repository: article_repository, comment_repository: comment_repository) }
